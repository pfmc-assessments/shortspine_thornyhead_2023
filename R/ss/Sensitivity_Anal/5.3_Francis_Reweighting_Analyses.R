...
# /Users/jzahner/Desktop/Projects/shortspine_thornyhead_2023/R/ss/Sensitivity_Anal/5.3_Francis_Reweighting_Outputs.R 
...
#  3. Developing model 23.model.francis  ----
# ----------------------------------------------------------- #

Dir_23_base_model <- file.path(dir_SensAnal, '5.2_Explore_FleetStructure', '5_23.model.fleetstruct_5', 'run', fsep=fsep)
replist <- SS_output(
...
Dir_23_model_francis <- file.path(dir_SensAnal, '5.3_Francis_Reweighting','1_23.model.francis' ,fsep = fsep)

# Add the model directory to the saved variables
var.to.save <- c(var.to.save, 'Dir_23_model_francis') 


# For each SS input file, the following variable names will be used:
# Starter file :			 Start23_model_francis 
# Data file :			 Dat23_model_francis 
# Control file :			 Ctl23_model_francis 
# Forecast file :			 Fore23_model_francis 
...
# ======================= #
# Read in the file
StarterFile <- file.path(Dir_23_model_francis, 'starter.ss', fsep = fsep)
Start23_model_francis <- SS_readstarter(
      file = StarterFile,
      verbose = TRUE
      )
...
# SS_writestarter(
      # mylist =  Start23_model_francis ,
      # dir =  Dir_23_model_francis, 
      # overwrite = TRUE,
      # verbose = TRUE
      # )

# Check file structure
# StarterFile <- file.path(Dir_23_model_francis, 'starter.ss')
#  Start23_model_francis <- SS_readstarter(
      # file = StarterFile, 
      # verbose = TRUE
      # )
...
# clean environment
var.to.save <- c(var.to.save, 'Start23_model_francis')
...
# ======================= #
# Read in the file
DatFile <-file.path(Dir_23_model_francis,Start23_model_francis$datfile, fsep = fsep)
Dat23_model_francis <- SS_readdat_3.30(
      file = DatFile,
      verbose = TRUE,
      section = TRUE
      )
...
# SS_writedat(
      # datlist =  Dat23_model_francis ,
      # outfile = file.path(Dir_23_model_francis, 'SST_data.ss', fsep = fsep),
      # version = '3.30',
      # overwrite = TRUE
      # )

# Check file structure
# DatFile <- file.path(Dir_23_model_francis, 'SST_data.ss', fsep = fsep)
#  Dat23_model_francis <-
      # SS_readdat_3.30(
      # file = DatFile,
      # verbose = TRUE,
...
# clean environment
var.to.save <- c(var.to.save, 'Dat23_model_francis')
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
# Read in the file
Ctlfile <-file.path(Dir_23_model_francis,Start23_model_francis$ctlfile, fsep = fsep)
Ctl23_model_francis <- SS_readctl_3.30(
      file = Ctlfile,
      use_datlist = TRUE,
      datlist = file.path(Dir_23_model_francis, 'run','data_echo.ss_new', fsep = fsep),
      verbose = TRUE
      )
...
SS_writectl(
  ctllist =  Ctl23_model_francis ,
  outfile = file.path(Dir_23_model_francis, 'SST_control.ss', fsep = fsep),
  version = '3.30',
  overwrite = TRUE
)
# Check file structure
...
# clean environment
var.to.save <- c(var.to.save, 'Ctl23_model_francis')
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
# Read in the file
ForeFile <- file.path(Dir_23_model_francis, 'forecast.ss', fsep = fsep)
Fore23_model_francis <-SS_readforecast(
      file = ForeFile,
      version = '3.30',
      verbose = T,
      readAll = T
...
# clean environment
var.to.save <- c(var.to.save, 'Fore23_model_francis')
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
run_SS(SS_version = '3.30.21',
      # version of SS
      base_path = Dir_23_model_francis, 
...
Dirplot <- file.path(Dir_23_model_francis, 'run', fsep = fsep)
...
# This can be done using the 5.3_Francis_Reweighting_Outputs.R 
