...
# 	- Sensitivity to weight-length relationship 
# Author:  Team SST 
# Date: 2023-06-06 18:29:26 
...
#	 C:/Users/sgbey/Documents/Github/shortspine_thornyhead_2023/model/Sensitivity_Anal/STAR_Panel/4.4_WL_Sensitivity 
# 
# Features: 
#  Sensitivity to WL relationship to removing fish <=16 cm 
# ============================================================ #
...
# C:/Users/sgbey/Documents/Github/shortspine_thornyhead_2023/R/ss/Sensitivity_Anal/4.4_WL_Sensitivity_Outputs.R 
...
#  3. Developing model 23.WL.sensitivity  ----
# ----------------------------------------------------------- #

# Path to the 23.WL.sensitivity repertory
Dir_23_WL_sensitivity <- file.path(dir_SensAnal, '4.4_WL_Sensitivity','1_23.WL.sensitivity' ,fsep = fsep)

# Add the model directory to the saved variables
var.to.save <- c(var.to.save, 'Dir_23_WL_sensitivity') 


# For each SS input file, the following variable names will be used:
# Starter file :			 Start23_WL_sensitivity 
# Data file :			 Dat23_WL_sensitivity 
# Control file :			 Ctl23_WL_sensitivity 
# Forecast file :			 Fore23_WL_sensitivity 
...
# ======================= #
# Read in the file
StarterFile <- file.path(Dir_23_WL_sensitivity, 'starter.ss', fsep = fsep)
Start23_WL_sensitivity <- SS_readstarter(
      file = StarterFile,
      verbose = TRUE
      )
...
SS_writestarter(
      mylist =  Start23_WL_sensitivity ,
      dir =  Dir_23_WL_sensitivity, 
      overwrite = TRUE,
      verbose = TRUE
      )

# Check file structure
# StarterFile <- file.path(Dir_23_WL_sensitivity, 'starter.ss')
#  Start23_WL_sensitivity <- SS_readstarter(
      # file = StarterFile,
      # verbose = TRUE
      # )
...
# clean environment
var.to.save <- c(var.to.save, 'Start23_WL_sensitivity')
...
# ======================= #
# Read in the file
DatFile <-file.path(Dir_23_WL_sensitivity,Start23_WL_sensitivity$datfile, fsep = fsep)
Dat23_WL_sensitivity <- SS_readdat_3.30(
      file = DatFile,
      verbose = TRUE,
      section = TRUE
      )
...
SS_writedat(
      datlist =  Dat23_WL_sensitivity ,
      outfile = file.path(Dir_23_WL_sensitivity, 'SST_data.ss', fsep = fsep),
      version = '3.30',
      overwrite = TRUE
      )

# Check file structure
# DatFile <- file.path(Dir_23_WL_sensitivity, 'SST_data.ss', fsep = fsep)
#  Dat23_WL_sensitivity <-
      # SS_readdat_3.30(
      # file = DatFile,
      # verbose = TRUE,
      # section = TRUE
...
# clean environment
var.to.save <- c(var.to.save, 'Dat23_WL_sensitivity')
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
# Read in the file
Ctlfile <-file.path(Dir_23_WL_sensitivity,Start23_WL_sensitivity$ctlfile, fsep = fsep)
Ctl23_WL_sensitivity <- SS_readctl_3.30(
      file = Ctlfile,
      use_datlist = TRUE,
      datlist = file.path(Dir_23_WL_sensitivity, 'run','data_echo.ss_new', fsep = fsep),
      verbose = TRUE
      )
...
#females alpha
Ctl23_WL_sensitivity$MG_parms["Wtlen_1_Fem_GP_1",]$INIT <- 4.86e-6
#female beta
Ctl23_WL_sensitivity$MG_parms["Wtlen_2_Fem_GP_1",]$INIT <- 3.26

#male alpha
Ctl23_WL_sensitivity$MG_parms["Wtlen_1_Mal_GP_1",]$INIT <- 4.96e-6
#male beta
Ctl23_WL_sensitivity$MG_parms["Wtlen_2_Mal_GP_1",]$INIT <- 3.25


# Save the control file for the model
SS_writectl(
      ctllist =  Ctl23_WL_sensitivity ,
      outfile = file.path(Dir_23_WL_sensitivity, 'SST_control.ss', fsep = fsep),
      version = '3.30',
      overwrite = TRUE
      )
# Check file structure
...
# clean environment
var.to.save <- c(var.to.save, 'Ctl23_WL_sensitivity')
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
# Read in the file
ForeFile <- file.path(Dir_23_WL_sensitivity, 'forecast.ss', fsep = fsep)
Fore23_WL_sensitivity <-SS_readforecast(
      file = ForeFile,
      version = '3.30',
      verbose = T,
      readAll = T
...
# clean environment
var.to.save <- c(var.to.save, 'Fore23_WL_sensitivity')
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
Dirplot <- file.path(Dir_23_WL_sensitivity, 'run', fsep = fsep)
...
# This can be done using the 4.4_WL_Sensitivity_Outputs.R script.
