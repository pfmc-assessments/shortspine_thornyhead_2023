...
#  SSplotComparisons(states,png=TRUE,
  ...
  #### revised on December 26, 2013 base with Category 2 Sigma=0.72 and P* = 0.40:
  fore.base.SPR.5b <- SS_output(file.path('c:/SS/Thornyheads/runs/forecasts/',
                                          'cat2_and_DeVore_Dec2013/SST_base_SPR50_Pstar40'),
                                covar=FALSE)
  #### end revision
  ...
  fore.fixed <- SS_readforecast("forecasts/SST_base_Average_catch/forecast.ss",
                                Nfleets=4,Nareas=1)
  #### revised on December 26, 2013 base with Category 2 Sigma=0.72 and P* = 0.40:
  fore.fixed.5b <- SS_readforecast(file.path('c:/SS/Thornyheads/runs/forecasts/',
                                            'cat2_and_DeVore_Dec2013/SST_low_HighCatch',
                                            'forecast.ss'),
                                  Nfleets=4,Nareas=1)
  #### end revision
  ...
  fore.fixed.5b.catch <-
    data.frame(yr=2013:2024,
               fore.base.SPR.5b$timeseries[fore.base.SPR.5b$timeseries$Yr %in% 2013:2024,
                                           grep("dead(B)",
                                                names(fore.base.SPR.5b$timeseries),
                                                fixed=TRUE)])
  #### end revision
  ...
  # set fixed catch for SPR = 50% forecasts
  for(f in 1:4){
    fore.fixed.5b$ForeCatch$Catch_or_F[fore.fixed.5b$ForeCatch$Fleet==f] <-
      round(fore.fixed.5b.catch[[paste("dead.B.._",f,sep="")]])
  }
  #### end revision
  ...
  #### revised on December 26, 2013 base with Category 2 Sigma=0.72 and P* = 0.40:
  SS_writeforecast(fore.fixed.5b,
                   dir=file.path('c:/SS/Thornyheads/runs/forecasts/',
                       'cat2_and_DeVore_Dec2013/SST_low_HighCatch'),
                   overwrite=TRUE)
  SS_writeforecast(fore.fixed.5b,
                   dir=file.path('c:/SS/Thornyheads/runs/forecasts/',
                       'cat2_and_DeVore_Dec2013/SST_high_HighCatch'),
                   overwrite=TRUE)
  #### end revision
...
# now run models!
  ...
  fore.low.HighNew  <- SS_output(file.path('c:/SS/Thornyheads/runs/forecasts/',
                                           'cat2_and_DeVore_Dec2013/SST_low_HighCatch'),
                                           covar=FALSE)
  ...
  fore.high.HighNew  <- SS_output(file.path('c:/SS/Thornyheads/runs/forecasts/',
                                           'cat2_and_DeVore_Dec2013/SST_high_HighCatch'),
                                           covar=FALSE)

  # confirm matching catch series for Dec. 2013 revision
  cbind(2012:2024,
        round(tail(apply(fore.base.SPR.5b$timeseries[,grep("dead(B)",names(fore.base.SPR.5b$timeseries),fixed=TRUE)],1,sum),13)),
        round(tail(apply(fore.low.HighNew$timeseries[,grep("dead(B)",names(fore.low.HighNew$timeseries),fixed=TRUE)],1,sum),13)),
        round(tail(apply(fore.high.HighNew$timeseries[,grep("dead(B)",names(fore.high.HighNew$timeseries),fixed=TRUE)],1,sum),13)))
  ...
  # revised Dec. 2013
  big.table.newrows <- cbind(get.decision.stuff(fore.low.HighNew),
                             get.decision.stuff(fore.base.SPR.5b),
                             get.decision.stuff(fore.high.HighNew))
  write.csv(big.table.newrows, "c:/SS/Thornyheads/runs/forecasts/SST_decision_table_HighCatch_Dec2013.csv")
  ...
  pstar <- SS_output("forecasts/SST_base_SPR_0.5_newP-star")

  cbind(tail(apply(pstar$timeseries[,grep("dead(B)",names(pstar$timeseries),fixed=TRUE)],1,sum),12),
        tail(apply(pstar$timeseries[,grep("retain(B)",names(pstar$timeseries),fixed=TRUE)],1,sum),12))

  tmp <- pstar$timeseries[,c("Yr","Bio_smry","SpawnBio")]
  tmp$Depl <- tmp$SpawnBio/tmp$SpawnBio[1]
  tail(tmp,12)

  data.frame(yr=tail(tmp$Yr,12),
             OFL=pstar$derived_quants$Value[grep("OFLCatch_",pstar$derived_quants$LABEL)],
             tail(cbind(apply(pstar$timeseries[,grep("dead(B)",names(pstar$timeseries),fixed=TRUE)],1,sum),
                        apply(pstar$timeseries[,grep("retain(B)",names(pstar$timeseries),fixed=TRUE)],1,sum)),
                  12),
             tail(tmp[,-1],12))
  ...
  for(i in 2:6){
    ...
    retroSummary$Bratio[[i]] <- retroSummary$SpawnBio[[i]][-(1:3)]/retroSummary$SpawnBio[[i]][1]
    retroSummary$BratioUpper[[i]] <- retroSummary$Bratio[[i]]
    retroSummary$BratioLower[[i]] <- retroSummary$Bratio[[i]]
  }
  ...
  SSplotComparisons(retroSummary, endyrvec=endyrvec,uncertainty=TRUE,
                    ...
                    indexfleets=9,indexUncertainty=TRUE,indexQlabel=TRUE,indexQdigits=2)
  ...
  h0.4 <- SS_output('SST_base_FINAL_profile.h', repfile="Report2.sso",
                    covar=FALSE,compfile=NA,forecast=FALSE,NoCompOK=TRUE)
  h0.8 <- SS_output('SST_base_FINAL_profile.h', repfile="Report6.sso",
                    covar=FALSE,compfile=NA,forecast=FALSE,NoCompOK=TRUE)
  ...
  M0.04 <- SS_output('SST_base_FINAL_profile.M', repfile="Report3.sso",
                     covar=FALSE,compfile=NA,forecast=FALSE,NoCompOK=TRUE)
  M0.06 <- SS_output('SST_base_FINAL_profile.M', repfile="Report5.sso",
                     covar=FALSE,compfile=NA,forecast=FALSE,NoCompOK=TRUE)
  ...
  # make timeseries plots comparing models in profile
  SSplotComparisons(prof.Lmax.summary,legendlabels=c(paste("L =",Lmax.vec[c(3,1,5)],"cm",c("(Base model)","",""))),
                    models=c(3,1,5),legendorder=c(2,1,3),
                    pheight=4.5,png=TRUE,plotdir=mydir,legendloc='bottomleft')
  ...
  #########################################################
  ### Sensitivities to catch reconstruction
  #########################################################
  recon <- SS_output('SST_base_FINAL_7_catch_recon')
  recon.summary <- SSsummarize(list(sbase,recon))
  SSplotComparisons(recon.summary,legendlabels=c("Base model",
                                   "Model with catch reconstruction for pre-PacFIN years"),
                    pheight=4.5,png=TRUE,plotdir='SST_base_FINAL_7_catch_recon',
                    legendloc='bottomleft')

  plot(sbase$sprseries$Year[sbase$sprseries$Year<=2012],
       sbase$sprseries$Retain_Catch[sbase$sprseries$Year<=2012],
       type='h',lend=3,lwd=5,col='grey40',yaxs='i',
       ylim=c(0,5000),ylab="Landings (mt)", xlab="Year")
  lines(recon$sprseries$Year[recon$sprseries$Year<=2012],
        recon$sprseries$Retain_Catch[recon$sprseries$Year<=2012],
        lwd=2,col=rgb(1,0,0,.7),lty='41')

  png('SST_base_FINAL_7_catch_recon/catch_series.png',width=7,height=4,units='in',res=300)
  par(mar=c(5,4,1,1))
  plot(sbase$sprseries$Year[sbase$sprseries$Year<=2012],
       sbase$sprseries$Retain_Catch[sbase$sprseries$Year<=2012],
       type='n',lwd=3,col=4,yaxs='i',xaxs='i',
       ylim=c(0,5000),ylab="Landings (mt)", xlab="Year")
  polygon(c(1901,sbase$sprseries$Year[sbase$sprseries$Year<=2012],2012),
       c(-10,sbase$sprseries$Retain_Catch[sbase$sprseries$Year<=2012],-10),
       lwd=3,col=rgb(0,0,1,.1),border=NA)
  lines(sbase$sprseries$Year[sbase$sprseries$Year<=2012],
       sbase$sprseries$Retain_Catch[sbase$sprseries$Year<=2012],
       lwd=3,col=4)
  lines(recon$sprseries$Year[recon$sprseries$Year<=2012],
        recon$sprseries$Retain_Catch[recon$sprseries$Year<=2012],
        lwd=3,col=rgb(1,0,0,.9),lty='22')
  legend('topleft',legend=c("Base model (early catch from previous assessment)",
                     "Alternative catch based on catch reconstructions"),
         col=c(4,2),lwd=3,lty=c('91','22'),bty='n')
  dev.off()
  ...
  #########################################################
  altmods <- list(sbase,      # base
                  low.mod2,   # low state of nature (R0 = 9.7)
                  high.mod2,  # high state of nature (R0 = 11.2)
                  h0.4,       # h = 0.4
                  h0.8,       # h = 0.8
                  M0.04,      # M = 0.04
                  M0.06,      # M = 0.06
                  mat.middle, # alternative maturity 1
                  mat.crazy,  # alternative maturity 2
                  sigR.0,      # no devs
                  sigR.25,    # low devs
                  sigR.75,    # high devs
                  recon)      # catch reconstruction

  altnames <- c("Base model",
                "Low state of nature: log(R0)=9.7",
                "High state of nature: log(R0)=11.2",
                "Low steepness: h=0.4",
                "High steepness: h=0.8",
                "Low mortality: M=0.04",
                "High mortality: M=0.06",
                "Alternative maturity 1",
                "Alternative maturity 2",
                "No recruit variability",
                "Low recruit var.: sigmaR=0.25",
                "High recruit var.: sigmaR=0.75",
                "Alternative early catch")

  altsum <- SSsummarize(altmods)
  basesum <- SSsummarize(list(sbase))
  basesum$minbthreshs <- -999
  basesum$btargs <- -999
  SSplotComparisons(basesum,subplot=4,legend=FALSE,new=FALSE,
                    spacepoints=251,staggerpoints=0,lwd=5)
  SSplotComparisons(altsum,subplot=3,legendlabels=altnames,legendloc='bottomleft',
                    spacepoints=200,staggerpoints=1,new=FALSE,add=TRUE,lty=c(1:7,1:6))
  SSplotComparisons(basesum,subplot=3,legend=FALSE,lwd=5,
                    spacepoints=251,staggerpoints=0,new=FALSE,add=TRUE)

  png('../figs/big_comparison.png',width=7,height=7,units='in',res=300)
  SSplotComparisons(altsum,subplot=3,legendlabels=altnames,legendloc='left',
                    lwd=c(5,rep(3,12)),pch=rep(-1,13),
                    spacepoints=pi,staggerpoints=1,new=FALSE,lty=c(1:5,2:5,2:5))
  SSplotComparisons(basesum,subplot=4,legend=FALSE,lwd=5,col=rich.colors.short(14)[2],
                    spacepoints=pi,staggerpoints=251,new=FALSE,add=TRUE)
  dev.off()

  tab1 <-
    SStableComparisons(altsum, models = "all",
  ...
  tab2 <- tab1
  names(tab2) <- c("Label", altnames)
  for(icol in ncol(tab2):2){
    tab2[1:5,icol] <- tab2[1:5,icol] - tab2[1:5,2]
  }
  tab2[tab2$Label=="R0_billions",-1] <- 1e3*tab2[tab2$Label=="R0_billions",-1]
  tab2$Label[tab2$Label=="R0_billions"] <- "R0_millions"
  write.csv(tab2,'../tables/big_comparison.csv')
