...
# /Users/andrea/Desktop/shortspine_thornyhead_2023/R/ss/Sensitivity_Anal/5.15_Length_Comp_Reweight_Outputs.R 
...
#  3. Developing model 23.length.comp.reweight  ----
# ----------------------------------------------------------- #

# Path to the 23.length.comp.reweight repertory
Dir_23_length_comp_reweight <- file.path(dir_SensAnal, '5.15_Length_Comp_Reweight','1_23.length.comp.reweight' ,fsep = fsep)

# Add the model directory to the saved variables
var.to.save <- c(var.to.save, 'Dir_23_length_comp_reweight') 


# For each SS input file, the following variable names will be used:
# Starter file :			 Start23_length_comp_reweight 
# Data file :			 Dat23_length_comp_reweight 
# Control file :			 Ctl23_length_comp_reweight 
# Forecast file :			 Fore23_length_comp_reweight 
...
Restart_SA_modeldvpt(
      ...
      curr.model = '23.length.comp.reweight',
      files = 'all')
...
# ======================= #
# Read in the file
StarterFile <- file.path(Dir_23_length_comp_reweight, 'starter.ss', fsep = fsep)
Start23_length_comp_reweight <- SS_readstarter(
      file = StarterFile,
      verbose = TRUE
      )
...
SS_writestarter(
      mylist =  Start23_length_comp_reweight ,
      dir =  Dir_23_length_comp_reweight, 
      overwrite = TRUE,
      verbose = TRUE
      )

# Check file structure
# StarterFile <- file.path(Dir_23_length_comp_reweight, 'starter.ss')
#  Start23_length_comp_reweight <- SS_readstarter(
      # file = StarterFile,
      # verbose = TRUE
      # )
...
# clean environment
var.to.save <- c(var.to.save, 'Start23_length_comp_reweight')
...
# ======================= #
# Read in the file
DatFile <-file.path(Dir_23_length_comp_reweight,Start23_length_comp_reweight$datfile, fsep = fsep)
Dat23_length_comp_reweight <- SS_readdat_3.30(
      file = DatFile,
      verbose = TRUE,
      section = TRUE
      )
...
SS_writedat(
      datlist =  Dat23_length_comp_reweight ,
      outfile = file.path(Dir_23_length_comp_reweight, 'SST_data.ss', fsep = fsep),
      version = '3.30',
      overwrite = TRUE
      )

# Check file structure
# DatFile <- file.path(Dir_23_length_comp_reweight, 'SST_data.ss', fsep = fsep)
#  Dat23_length_comp_reweight <-
      # SS_readdat_3.30(
      # file = DatFile,
      # verbose = TRUE,
      # section = TRUE
...
# clean environment
var.to.save <- c(var.to.save, 'Dat23_length_comp_reweight')
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
# Read in the file
Ctlfile <-file.path(Dir_23_length_comp_reweight,Start23_length_comp_reweight$ctlfile, fsep = fsep)
Ctl23_length_comp_reweight <- SS_readctl_3.30(
      file = Ctlfile,
      use_datlist = TRUE,
      datlist = file.path(Dir_23_length_comp_reweight, 'run','data_echo.ss_new', fsep = fsep),
      verbose = TRUE
      )
...
SS_writectl(
      ctllist =  Ctl23_length_comp_reweight ,
      outfile = file.path(Dir_23_length_comp_reweight, 'SST_control.ss', fsep = fsep),
      version = '3.30',
      overwrite = TRUE
      )
# Check file structure
...
# clean environment
var.to.save <- c(var.to.save, 'Ctl23_length_comp_reweight')
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
# Read in the file
ForeFile <- file.path(Dir_23_length_comp_reweight, 'forecast.ss', fsep = fsep)
Fore23_length_comp_reweight <-SS_readforecast(
      file = ForeFile,
      version = '3.30',
      verbose = T,
      readAll = T
...
SS_writeforecast(
      mylist =  Fore23_length_comp_reweight ,
      dir = Dir_23_length_comp_reweight, 
      file = 'forecast.ss',
      writeAll = TRUE,
      verbose = TRUE,
...
# clean environment
var.to.save <- c(var.to.save, 'Fore23_length_comp_reweight')
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
Dirplot <- file.path(Dir_23_length_comp_reweight, 'run', fsep = fsep)
...
# This can be done using the 5.15_Length_Comp_Reweight_Outputs.R script.
