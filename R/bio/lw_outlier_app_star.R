...
# modified original outlier app to only use combo survey data and also identify
# outliers in log-log space
...
server <- function(input, output) {
  ...
  dat <- combo %>% mutate(survey = 'NWFSCcombo', 
  ...
  output$LengthWeight <- renderPlotly({
    ...
    gg1 = key %>% 
      ggplot(aes(x = log(Length_cm), y = log(Weight), shape = Sex, colour = Sex, text = rn, customdata = rn)) +
      geom_point() 
