...
rownames(summary) <- c("Total L", "Survey L", "Length Comp L", "Discards L", 
                       "Mean Body Wt L", "Recruitment L", "Prior L", "R0", "B0",
                       "Depletion", "Relative SPR")
...
# ---------------------------------------------------------------------------- #
### Including Natural mortality SA in the summary table
...
model.ids <- c("Base_Model/5.23_Official_Base/1_23.base.official",
              #"5.8_Francis_Reweighting_2/1_23.model.francis_2", 
               ...
               "4.2_Maturity_Sensitivity/2_23.maturity.mix_curve",
               "STAR_Panel/4.4_WL_Sensitivity/1_23.WL.sensitivity",
               "STAR_Panel/5.13_Request_3/1_23.STAR.Panel.M.045",
               "STAR_Panel/5.13_Request_3/2_23.STAR.Panel.M.05",
               "1.1_Landings_Sensitivity/1_23.land.hist_impute", 
               "1.1_Landings_Sensitivity/2_23.land.2013",
               "1.2_Landings_ashop/1_23.landings.ashop",
               "3.1_surveys_Sensitivity/1_23.surveys.gamvln",
...
summary_table %>%
  kbl() %>%
  kable_classic(full_width = F, html_font = "Times New Roman") %>%
  add_header_above(c(" " = 1, " " = 1, "Biology" = 7, "Landings" = 3, 
                     "Surveys" = 5, "Retention + Selectivity" = 3)) %>% 
  save_kable(file = file.path(here::here(), "outputs", "summary_table_W_NatM.jpeg"),
             zoom = 2)
