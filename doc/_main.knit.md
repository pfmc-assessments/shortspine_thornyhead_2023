...
where:
$L_{a_{1}}$ and $L_{a_{2}}$ are the lengths at reference ages $a_{1}$ and $a_{2}$ where $a_{1}=1$;$a_{2}=100$ and k is the growth rate. Growth curve estimation was conducted in R using the optim() function [@r_core_2016]. Errors were assumed to be lognormally distributed, and predicted length-at-age was bias-corrected using a multiplier of 𝜎2 / 2.
