...
#	 /Users/jzahner/Desktop/Projects/shortspine_thornyhead_2023/model/Sensitivity_Anal/1.2_Landings_ashop 
# 
# Features: 
#  Integrating At-Sea Hake catches of SST. This data was provided in May and processed by Kiva Oken. 
# ============================================================ #
...
# /Users/jzahner/Desktop/Projects/shortspine_thornyhead_2023/R/ss/Sensitivity_Anal/1.2_Landings_ashop_Outputs.R 
...
# won't be estimated.
# Reminder - The following models are considered:# 	-  23.landings.ashop 
noHess <- c(TRUE)
...
#  3. Developing model 23.landings.ashop  ----
# ----------------------------------------------------------- #

# Path to the 23.landings.ashop repertory
Dir_23_landings_ashop <- file.path(dir_SensAnal, '1.2_Landings_ashop','1_23.landings.ashop' ,fsep = fsep)

# Add the model directory to the saved variables
var.to.save <- c(var.to.save, 'Dir_23_landings_ashop') 


# For each SS input file, the following variable names will be used:
# Starter file :			 Start23_landings_ashop 
# Data file :			 Dat23_landings_ashop 
# Control file :			 Ctl23_landings_ashop 
# Forecast file :			 Fore23_landings_ashop 
...
# ======================= #
# Read in the file
StarterFile <- file.path(Dir_23_landings_ashop, 'starter.ss', fsep = fsep)
Start23_landings_ashop <- SS_readstarter(
      file = StarterFile,
      verbose = TRUE
      )
...
# SS_writestarter(
      # mylist =  Start23_landings_ashop ,
      # dir =  Dir_23_landings_ashop, 
      # overwrite = TRUE,
      # verbose = TRUE
      # )

# Check file structure
# StarterFile <- file.path(Dir_23_landings_ashop, 'starter.ss')
#  Start23_landings_ashop <- SS_readstarter(
      # file = StarterFile,
      # verbose = TRUE
      # )
...
# clean environment
var.to.save <- c(var.to.save, 'Start23_landings_ashop')
...
# ======================= #
# Read in the file
DatFile <-file.path(Dir_23_landings_ashop,Start23_landings_ashop$datfile, fsep = fsep)
Dat23_landings_ashop <- SS_readdat_3.30(
      file = DatFile,
      verbose = TRUE,
      section = TRUE
      )
...
# ..... 
# ..... 
Dat23_landings_ashop$catch <- SS_Param2023$Catch$data$ThreeFleets_NoSlope_CombineTriennial_ASHOP %>% rename(seas=season)

# Save the data file for the model
SS_writedat(
datlist =  Dat23_landings_ashop ,
outfile = file.path(Dir_23_landings_ashop, 'SST_data.ss', fsep = fsep),
version = '3.30',
overwrite = TRUE
)

# Check file structure
# DatFile <- file.path(Dir_23_landings_ashop, 'SST_data.ss', fsep = fsep)
#  Dat23_landings_ashop <-
      # SS_readdat_3.30(
      # file = DatFile,
      # verbose = TRUE,
      # section = TRUE
...
# clean environment
var.to.save <- c(var.to.save, 'Dat23_landings_ashop')
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
# Read in the file
Ctlfile <-file.path(Dir_23_landings_ashop,Start23_landings_ashop$ctlfile, fsep = fsep)
Ctl23_landings_ashop <- SS_readctl_3.30(
      file = Ctlfile,
      use_datlist = TRUE,
      datlist = file.path(Dir_23_landings_ashop, 'run','data_echo.ss_new', fsep = fsep),
      verbose = TRUE
      )
...
# SS_writectl(
      # ctllist =  Ctl23_landings_ashop ,
      # outfile = file.path(Dir_23_landings_ashop, 'SST_control.ss', fsep = fsep),
      # version = '3.30',
      # overwrite = TRUE
      # )
# Check file structure
...
# clean environment
var.to.save <- c(var.to.save, 'Ctl23_landings_ashop')
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
# Read in the file
ForeFile <- file.path(Dir_23_landings_ashop, 'forecast.ss', fsep = fsep)
Fore23_landings_ashop <-SS_readforecast(
      file = ForeFile,
      version = '3.30',
      verbose = T,
      readAll = T
...
# clean environment
var.to.save <- c(var.to.save, 'Fore23_landings_ashop')
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
Dirplot <- file.path(Dir_23_landings_ashop, 'run', fsep = fsep)
...
# This can be done using the 1.2_Landings_ashop_Outputs.R script.
