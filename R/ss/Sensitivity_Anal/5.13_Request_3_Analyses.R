...
# C:/Users/Matthieu Verson/Documents/GitHub/Forked_repos/shortspine_thornyhead_2023/R/ss/Sensitivity_Anal/5.13_Request_3_Outputs.R 
...
#  3. Developing model 23.STAR.Panel.M.045  ----
# ----------------------------------------------------------- #

# Path to the 23.STAR.Panel.M.045 repertory
Dir_23_STAR_Panel_M_045 <- file.path(dir_SensAnal, '5.13_Request_3','1_23.STAR.Panel.M.045' ,fsep = fsep)

# Add the model directory to the saved variables
var.to.save <- c(var.to.save, 'Dir_23_STAR_Panel_M_045') 


# For each SS input file, the following variable names will be used:
# Starter file :			 Start23_STAR_Panel_M_045 
# Data file :			 Dat23_STAR_Panel_M_045 
# Control file :			 Ctl23_STAR_Panel_M_045 
# Forecast file :			 Fore23_STAR_Panel_M_045 
...
Restart_SA_modeldvpt(
      ...
      curr.model = '23.STAR.Panel.M.045',
      files = 'all')
...
# ======================= #
# Read in the file
StarterFile <- file.path(Dir_23_STAR_Panel_M_045, 'starter.ss', fsep = fsep)
Start23_STAR_Panel_M_045 <- SS_readstarter(
      file = StarterFile,
      verbose = TRUE
      )
...
SS_writestarter(
      mylist =  Start23_STAR_Panel_M_045 ,
      dir =  Dir_23_STAR_Panel_M_045, 
      overwrite = TRUE,
      verbose = TRUE
      )

# Check file structure
# StarterFile <- file.path(Dir_23_STAR_Panel_M_045, 'starter.ss')
#  Start23_STAR_Panel_M_045 <- SS_readstarter(
      # file = StarterFile,
      # verbose = TRUE
      # )
...
# clean environment
var.to.save <- c(var.to.save, 'Start23_STAR_Panel_M_045')
...
# ======================= #
# Read in the file
DatFile <-file.path(Dir_23_STAR_Panel_M_045,Start23_STAR_Panel_M_045$datfile, fsep = fsep)
Dat23_STAR_Panel_M_045 <- SS_readdat_3.30(
      file = DatFile,
      verbose = TRUE,
      section = TRUE
      )
...
SS_writedat(
      datlist =  Dat23_STAR_Panel_M_045 ,
      outfile = file.path(Dir_23_STAR_Panel_M_045, 'SST_data.ss', fsep = fsep),
      version = '3.30',
      overwrite = TRUE
      )

# Check file structure
# DatFile <- file.path(Dir_23_STAR_Panel_M_045, 'SST_data.ss', fsep = fsep)
#  Dat23_STAR_Panel_M_045 <-
      # SS_readdat_3.30(
      # file = DatFile,
      # verbose = TRUE,
      # section = TRUE
...
# clean environment
var.to.save <- c(var.to.save, 'Dat23_STAR_Panel_M_045')
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
# Read in the file
Ctlfile <-file.path(Dir_23_STAR_Panel_M_045,Start23_STAR_Panel_M_045$ctlfile, fsep = fsep)
Ctl23_STAR_Panel_M_045 <- SS_readctl_3.30(
      file = Ctlfile,
      use_datlist = TRUE,
      datlist = file.path(Dir_23_STAR_Panel_M_045, 'run','data_echo.ss_new', fsep = fsep),
      verbose = TRUE
      )
...
# Fix M at 0.045
Which1 <- which(row.names(Ctl23_STAR_Panel_M_045$MG_parms) %in% 
                  c("NatM_p_1_Fem_GP_1", "NatM_p_2_Fem_GP_1"))
Ctl23_STAR_Panel_M_045$MG_parms[Which1, "INIT"] <- 0.045


# Save the control file for the model
SS_writectl(
      ctllist =  Ctl23_STAR_Panel_M_045 ,
      outfile = file.path(Dir_23_STAR_Panel_M_045, 'SST_control.ss', fsep = fsep),
      version = '3.30',
      overwrite = TRUE
      )
# Check file structure
...
# clean environment
var.to.save <- c(var.to.save, 'Ctl23_STAR_Panel_M_045')
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
# Read in the file
ForeFile <- file.path(Dir_23_STAR_Panel_M_045, 'forecast.ss', fsep = fsep)
Fore23_STAR_Panel_M_045 <-SS_readforecast(
      file = ForeFile,
      version = '3.30',
      verbose = T,
      readAll = T
...
SS_writeforecast(
      mylist =  Fore23_STAR_Panel_M_045 ,
      dir = Dir_23_STAR_Panel_M_045, 
      file = 'forecast.ss',
      writeAll = TRUE,
      verbose = TRUE,
...
# clean environment
var.to.save <- c(var.to.save, 'Fore23_STAR_Panel_M_045')
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
Dirplot <- file.path(Dir_23_STAR_Panel_M_045, 'run', fsep = fsep)
...
# End development for model 23.STAR.Panel.M.045 
# You can now develop the next 'new model'.
# Let's remove the input files for this from the 'var.to.save' variable
var.to.save <- var.to.save[!var.to.save %in% c('Start23_STAR_Panel_M_045','Dat23_STAR_Panel_M_045','Ctl23_STAR_Panel_M_045','Fore23_STAR_Panel_M_045')]
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
...
#  4. Developing model 23.STAR.Panel.M.05  ----
# ----------------------------------------------------------- #

# Path to the 23.STAR.Panel.M.05 repertory
Dir_23_STAR_Panel_M_05 <- file.path(dir_SensAnal, '5.13_Request_3','2_23.STAR.Panel.M.05' ,fsep = fsep)

# Add the model directory to the saved variables
var.to.save <- c(var.to.save, 'Dir_23_STAR_Panel_M_05') 


# For each SS input file, the following variable names will be used:
# Starter file :					 Start23_STAR_Panel_M_05 
# Data file :					 Dat23_STAR_Panel_M_05 
# Control file :					 Ctl23_STAR_Panel_M_05 
# Forecast file :					 Fore23_STAR_Panel_M_05 
...
# ======================= #
# Read in the file
StarterFile <- file.path(Dir_23_STAR_Panel_M_05, 'starter.ss', fsep = fsep)
Start23_STAR_Panel_M_05 <- SS_readstarter(
      file = StarterFile,
      verbose = TRUE
      )
...
# Save the starter file for the model
SS_writestarter(
      mylist =  Start23_STAR_Panel_M_05 ,
      dir =  Dir_23_STAR_Panel_M_05, 
      overwrite = TRUE,
      verbose = TRUE
      )

# Check file structure
# StarterFile <- file.path(Dir_23_STAR_Panel_M_05, 'starter.ss')
#  Start23_STAR_Panel_M_05 <- SS_readstarter(
      # file = StarterFile,
      # verbose = TRUE
      # )
...
# clean environment
var.to.save <- c(var.to.save, 'Start23_STAR_Panel_M_05')
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
# ======================= #
# Read in the file
DatFile <-file.path(Dir_23_STAR_Panel_M_05,Start23_STAR_Panel_M_05$datfile, fsep = fsep)
Dat23_STAR_Panel_M_05 <- SS_readdat_3.30(
      file = DatFile,
      verbose = TRUE,
      section = TRUE
      )
...
# Save the data file for the model
SS_writedat(
      datlist =  Dat23_STAR_Panel_M_05 ,
      outfile = file.path(Dir_23_STAR_Panel_M_05, 'SST_data.ss', fsep = fsep),
      version = '3.30',
      overwrite = TRUE
      )

# Check file structure
# DatFile <- file.path(Dir_23_STAR_Panel_M_05, 'SST_data.ss', fsep = fsep)
#  Dat23_STAR_Panel_M_05 <-
      # SS_readdat_3.30(
      # file = DatFile,
      # verbose = TRUE,
      # section = TRUE
...
# clean environment
var.to.save <- c(var.to.save, 'Dat23_STAR_Panel_M_05')
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
# Read in the file
Ctlfile <-file.path(Dir_23_STAR_Panel_M_05,Start23_STAR_Panel_M_05$ctlfile, fsep = fsep)
Ctl23_STAR_Panel_M_05 <- SS_readctl_3.30(
      file = Ctlfile,
      use_datlist = TRUE,
      datlist = file.path(Dir_23_STAR_Panel_M_05, 'run','data_echo.ss_new', fsep = fsep),
      verbose = TRUE
      )
...
# Fix M at 0.05
Which1 <- which(row.names(Ctl23_STAR_Panel_M_05$MG_parms) %in% 
                  c("NatM_p_1_Fem_GP_1", "NatM_p_2_Fem_GP_1"))
Ctl23_STAR_Panel_M_05$MG_parms[Which1, "INIT"] <- 0.05


# Save the control file for the model
SS_writectl(
      ctllist =  Ctl23_STAR_Panel_M_05 ,
      outfile = file.path(Dir_23_STAR_Panel_M_05, 'SST_control.ss', fsep = fsep),
      version = '3.30',
      overwrite = TRUE
      )
# Check file structure
...
# clean environment
var.to.save <- c(var.to.save, 'Ctl23_STAR_Panel_M_05')
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
# Read in the file
ForeFile <- file.path(Dir_23_STAR_Panel_M_05, 'forecast.ss', fsep = fsep)
Fore23_STAR_Panel_M_05 <-SS_readforecast(
      file = ForeFile,
      version = '3.30',
      verbose = T,
      readAll = T
...
# clean environment
var.to.save <- c(var.to.save, 'Fore23_STAR_Panel_M_05')
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
run_SS(SS_version = '3.30.21',
      # version of SS
      base_path =Dir_23_STAR_Panel_M_05,
      # root directory where the input file are housed
      pathRun = NULL,
      # A 'run' folder is created if needed (where output files will be stored)
      copy_files = TRUE,
      # copy the input files from the23.STAR.Panel.M.05folder
      cleanRun = TRUE,
      # clean the folder after the run
...
# ======================= #
# read the model output and print diagnostic messages
Dirplot <- file.path(Dir_23_STAR_Panel_M_05, 'run', fsep = fsep)
...
# This can be done using the 5.13_Request_3_Outputs.R script.
