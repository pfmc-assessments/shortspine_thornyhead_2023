...
SA_path <- file.path('model','Sensitivity_Anal','STAR_Panel','4.5_SigmaR_Sensitivity',fsep = fsep)

# Path to the 23.SigmaR.sensitivity repertory
Dir_23_SigmaR_sensitivity <- file.path(dir_SensAnal, '4.5_SigmaR_Sensitivity','1_23.SigmaR.sensitivity' , 'run',fsep = fsep)

# Extract the outputs for all models
SensiMod <- SSgetoutput(dirvec = c(
	Dir_23_model_francis_2,
	Dir_23_SigmaR_sensitivity))
