if(FALSE){
  ...
  SS_plots(s2.11,png=TRUE,datplot=TRUE,
           fleetnames=fleets)
...
pngfun <- function(file,w=7,h=7,pt=12){
  file <- file.path('C:/ss/thornyheads/figs',file)
...
#  SSplotData(sbase, print=TRUE, fleetnames=fleets, margins = c(5.1, 2.1, 4.1, 13.1),ptsize=9)
  ...
  # index data
  if(!wide){
    pngfun('SST_index_data_s6.2.png',h=8.5)
    par(mfrow=c(3,2),mar=c(2,2,2,1),oma=c(2,2,0,0)+.1)
  }else{
    pngfun('SST_index_data_s6.2.png',h=7,w=10)
    par(mfrow=c(2,3),mar=c(2,2,2,1),oma=c(2,2,0,0)+.1)
  }
  for(f in 5:9){
    SSplotIndices(ref,fleets=f,subplot=1,datplot=TRUE)
  }
  ...
  # index fits
  if(!wide){
    pngfun('SST_index_fits_s6.2.png',h=8.5)
    par(mfrow=c(3,2),mar=c(2,2,2,1),oma=c(2,2,0,0)+.1)
  }else{
    pngfun('SST_index_fits_s6.2.png',h=7,w=10)
    par(mfrow=c(2,3),mar=c(2,2,2,1),oma=c(2,2,0,0)+.1)
  }
  for(f in 5:9){
    SSplotIndices(ref,fleets=f,subplot=2)
  }
  mtext(side=1,line=1,outer=TRUE,'Year')
  mtext(side=2,line=1,outer=TRUE,'Index')
  dev.off()
  ...
  # index fits base
  if(!wide){
    pngfun('SST_index_fits_s2.0.png',h=8.5)
    par(mfrow=c(3,2),mar=c(2,2,2,1),oma=c(2,2,0,0)+.1)
  }else{
    pngfun('SST_index_fits_s2.0.png',h=7,w=10)
    par(mfrow=c(2,3),mar=c(2,2,2,1),oma=c(2,2,0,0)+.1)
  }
  for(f in 5:9){
  ...
  # index fits combined
  pngfun('SST_index_fits_compare.png',h=6,w=10,pt=18)
  par(mfcol=c(2,3),mar=c(2,2,2,1),oma=c(2,2,0,0)+.1)
  f <- 5
  SSplotIndices(sbase,fleets=f,subplot=2)
  SSplotIndices(s6.2,fleets=f,subplot=2,col3=rgb(1,0,0,.7))
  for(f in 6:9){
    SSplotIndices(sbase,fleets=f,subplot=2)
    SSplotIndices(s6.2,fleets=f,subplot=2,col3=rgb(1,0,0,.7),add=TRUE)
  }
  mtext(side=1,line=1,outer=TRUE,'Year')
  mtext(side=2,line=1,outer=TRUE,'Index')
  dev.off()  
  ...
  # discard fits
  pngfun('../figs/SST_discard_fits_s2.0.png')
  par(mfcol=c(2,2),mar=c(2,2,2,1),oma=c(2,2,0,0)+.1)
  for(f in 1:4){
    SSplotDiscard(sbase,fleets=f,subplot=2,fleetnames=fleets,pwidth=5)
  }
  mtext(side=1,line=1,outer=TRUE,'Year')
  mtext(side=2,line=1,outer=TRUE,'Fraction discarded')
  dev.off()

  pngfun('../figs/SST_discard_fits_s6.2.png')
  par(mfcol=c(2,2),mar=c(2,2,2,1),oma=c(2,2,0,0)+.1)
  for(f in 1:4){
    SSplotDiscard(ref,fleets=f,subplot=2,fleetnames=fleets,pwidth=5)
  }
  mtext(side=1,line=1,outer=TRUE,'Year')
  mtext(side=2,line=1,outer=TRUE,'Fraction discarded')
  dev.off()
