...
# sensitivity analysis: Item 5.18 
# ============================================================ #
# 
...
# 	- SigmaR_non_sum_to_zero 
# Author:  Joshua Zahner 
# Date: 2023-06-08 08:43:53 
...
#	 C:/Users/sgbey/Documents/Github/shortspine_thornyhead_2023/model/Sensitivity_Anal/STAR_Panel/5.18_SigmaR_non_sum_to_zero 
# 
# Features: 
#  Allow the recruit devs to not sum to zero 
# ============================================================ #
...
# C:/Users/sgbey/Documents/Github/shortspine_thornyhead_2023/R/ss/Sensitivity_Anal/5.18_SigmaR_non_sum_to_zero_Outputs.R 
...
#  3. Developing model 23.model.SigmaR.non.zero  ----
# ----------------------------------------------------------- #

# Path to the 23.model.SigmaR.non.zero repertory
Dir_23_model_SigmaR_non_zero <- file.path(dir_SensAnal, '5.18_SigmaR_non_sum_to_zero','1_23.model.SigmaR.non.zero' ,fsep = fsep)

# Add the model directory to the saved variables
var.to.save <- c(var.to.save, 'Dir_23_model_SigmaR_non_zero') 


# For each SS input file, the following variable names will be used:
# Starter file :			 Start23_model_SigmaR_non_zero 
# Data file :			 Dat23_model_SigmaR_non_zero 
# Control file :			 Ctl23_model_SigmaR_non_zero 
# Forecast file :			 Fore23_model_SigmaR_non_zero 
...
Restart_SA_modeldvpt(
      ...
      curr.model = '23.model.SigmaR.non.zero',
      files = 'all')
...
# ======================= #
# Read in the file
StarterFile <- file.path(Dir_23_model_SigmaR_non_zero, 'starter.ss', fsep = fsep)
Start23_model_SigmaR_non_zero <- SS_readstarter(
      file = StarterFile,
      verbose = TRUE
      )
...
SS_writestarter(
      mylist =  Start23_model_SigmaR_non_zero ,
      dir =  Dir_23_model_SigmaR_non_zero, 
      overwrite = TRUE,
      verbose = TRUE
      )

# Check file structure
# StarterFile <- file.path(Dir_23_model_SigmaR_non_zero, 'starter.ss')
#  Start23_model_SigmaR_non_zero <- SS_readstarter(
      # file = StarterFile,
      # verbose = TRUE
      # )
...
# clean environment
var.to.save <- c(var.to.save, 'Start23_model_SigmaR_non_zero')
...
# ======================= #
# Read in the file
DatFile <-file.path(Dir_23_model_SigmaR_non_zero,Start23_model_SigmaR_non_zero$datfile, fsep = fsep)
Dat23_model_SigmaR_non_zero <- SS_readdat_3.30(
      file = DatFile,
      verbose = TRUE,
      section = TRUE
      )
...
SS_writedat(
      datlist =  Dat23_model_SigmaR_non_zero ,
      outfile = file.path(Dir_23_model_SigmaR_non_zero, 'SST_data.ss', fsep = fsep),
      version = '3.30',
      overwrite = TRUE
      )

# Check file structure
# DatFile <- file.path(Dir_23_model_SigmaR_non_zero, 'SST_data.ss', fsep = fsep)
#  Dat23_model_SigmaR_non_zero <-
      # SS_readdat_3.30(
      # file = DatFile,
      # verbose = TRUE,
      # section = TRUE
...
# clean environment
var.to.save <- c(var.to.save, 'Dat23_model_SigmaR_non_zero')
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
# Read in the file
Ctlfile <-file.path(Dir_23_model_SigmaR_non_zero,Start23_model_SigmaR_non_zero$ctlfile, fsep = fsep)
Ctl23_model_SigmaR_non_zero <- SS_readctl_3.30(
      file = Ctlfile,
      use_datlist = TRUE,
      datlist = file.path(Dir_23_model_SigmaR_non_zero, 'run','data_echo.ss_new', fsep = fsep),
      verbose = TRUE
      )
...
SS_writectl(
      ctllist =  Ctl23_model_SigmaR_non_zero ,
      outfile = file.path(Dir_23_model_SigmaR_non_zero, 'SST_control.ss', fsep = fsep),
      version = '3.30',
      overwrite = TRUE
      )
# Check file structure
...
# clean environment
var.to.save <- c(var.to.save, 'Ctl23_model_SigmaR_non_zero')
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
# Read in the file
ForeFile <- file.path(Dir_23_model_SigmaR_non_zero, 'forecast.ss', fsep = fsep)
Fore23_model_SigmaR_non_zero <-SS_readforecast(
      file = ForeFile,
      version = '3.30',
      verbose = T,
      readAll = T
...
SS_writeforecast(
      mylist =  Fore23_model_SigmaR_non_zero ,
      dir = Dir_23_model_SigmaR_non_zero, 
      file = 'forecast.ss',
      writeAll = TRUE,
      verbose = TRUE,
...
# clean environment
var.to.save <- c(var.to.save, 'Fore23_model_SigmaR_non_zero')
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
Dirplot <- file.path(Dir_23_model_SigmaR_non_zero, 'run', fsep = fsep)
...
# This can be done using the 5.18_SigmaR_non_sum_to_zero_Outputs.R script.
