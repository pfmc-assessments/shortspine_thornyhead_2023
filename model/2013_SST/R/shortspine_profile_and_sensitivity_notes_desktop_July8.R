...
#  profile <- SS_profile(dir=mydir, # directory
  ...
  R0.vec <- seq(9,11,.2)
  Nprof.R0 <- length(R0.vec)
  # adding base model to the end of list of model outputs created by SSgetoutput
  prof.R0.models$MLE <- sbase
  # summarize stuff
  prof.R0.summary <- SSsummarize(prof.R0.models)
  ...
  plot(R0.vec2[1:11],B0vec[1:11],type='l',lwd=3,ylim=c(0,max(B0vec)),las=1,yaxs='i',
  ...
  plot(R0.vec2[1:11],deplvec[1:11],type='l',lwd=3,ylim=c(0,1),las=1,yaxs='i',
  ...
  abline(h=Qvec[12],lty=3,col='grey')
  abline(v=baseval,lty=3,col='grey')
  dev.off()
  ...
  SSplotPars("SST_2.0_profile.M",string="NatM",xlab="",showlegend=FALSE,showinit=FALSE,showmle=FALSE,showpost=FALSE,showrecdev=FALSE,new=FALSE)
  abline(v=0.0505,col=2)
  mtext(side=1,line=3,"Natural mortality (M)")
  SSplotPars("SST_2.0_profile.h",string="steep",xlab="",showlegend=FALSE,showinit=FALSE,showmle=FALSE,showpost=FALSE,showrecdev=FALSE,new=FALSE)
  ...
  mat.old <- SS_output('SST_2.0_maturity.old')
  mat.crazy <- SS_output('SST_2.0_maturity.crazy')
