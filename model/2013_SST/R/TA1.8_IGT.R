SSMethod.TA1.8 <-
function(fit,type,fleet,part=0:2,pick.gender=0:3,plotit=TRUE,new=TRUE,ylim=NULL,...)
...
#            mtext(lab,3,at=mean(x))
        ...
        mtext(paste('Mean',ifelse(type=='len','length','age')),2,out=T,las=0)
    ...
    confint <- quantile(apply(tmp,2,function(x)1/var(x,na.rm=T)),
                        c(0.025,0.975))
    print(c(w=Nmult,lo=confint[1],hi=confint[2]))
    return(invisible(data.frame(x=x[ord],obs=subpldat[ord,'Obsmn'],exp=subpldat[ord,'Expmn'])))
}
