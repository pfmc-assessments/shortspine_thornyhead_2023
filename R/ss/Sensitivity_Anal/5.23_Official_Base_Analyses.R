...
# /Users/jzahner/Desktop/shortspine_thornyhead_2023/R/ss/Sensitivity_Anal/5.23_Official_Base_Outputs.R 
...
#  3. Developing model 23.base.official  ----
# ----------------------------------------------------------- #

# Path to the 23.base.official repertory
Dir_23_base_official <- file.path(dir_SensAnal, '5.23_Official_Base','1_23.base.official' ,fsep = fsep)

# Add the model directory to the saved variables
var.to.save <- c(var.to.save, 'Dir_23_base_official') 


# For each SS input file, the following variable names will be used:
# Starter file :			 Start23_base_official 
# Data file :			 Dat23_base_official 
# Control file :			 Ctl23_base_official 
# Forecast file :			 Fore23_base_official 
...
# ======================= #
# Read in the file
StarterFile <- file.path(Dir_23_base_official, 'starter.ss', fsep = fsep)
Start23_base_official <- SS_readstarter(
      file = StarterFile,
      verbose = TRUE
      )
...
SS_writestarter(
      mylist =  Start23_base_official ,
      dir =  Dir_23_base_official, 
      overwrite = TRUE,
      verbose = TRUE
      )

# Check file structure
# StarterFile <- file.path(Dir_23_base_official, 'starter.ss')
#  Start23_base_official <- SS_readstarter(
      # file = StarterFile,
      # verbose = TRUE
      # )
...
# clean environment
var.to.save <- c(var.to.save, 'Start23_base_official')
...
# ======================= #
# Read in the file
DatFile <-file.path(Dir_23_base_official,Start23_base_official$datfile, fsep = fsep)
Dat23_base_official <- SS_readdat_3.30(
      file = DatFile,
      verbose = TRUE,
      section = TRUE
      )
...
SS_writedat(
      datlist =  Dat23_base_official ,
      outfile = file.path(Dir_23_base_official, 'SST_data.ss', fsep = fsep),
      version = '3.30',
      overwrite = TRUE
      )

# Check file structure
# DatFile <- file.path(Dir_23_base_official, 'SST_data.ss', fsep = fsep)
#  Dat23_base_official <-
      # SS_readdat_3.30(
      # file = DatFile,
      # verbose = TRUE,
...
# clean environment
var.to.save <- c(var.to.save, 'Dat23_base_official')
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
# Read in the file
Ctlfile <-file.path(Dir_23_base_official,Start23_base_official$ctlfile, fsep = fsep)
Ctl23_base_official <- SS_readctl_3.30(
      file = Ctlfile,
      use_datlist = TRUE,
      datlist = file.path(Dir_23_base_official, 'run','data_echo.ss_new', fsep = fsep),
      verbose = TRUE
      )
...
SS_writectl(
      ctllist =  Ctl23_base_official ,
      outfile = file.path(Dir_23_base_official, 'SST_control.ss', fsep = fsep),
      version = '3.30',
      overwrite = TRUE
      )
# Check file structure
...
# clean environment
var.to.save <- c(var.to.save, 'Ctl23_base_official')
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
# Read in the file
ForeFile <- file.path(Dir_23_base_official, 'forecast.ss', fsep = fsep)
...
Fore23_base_official <-SS_readforecast(
      file = ForeFile,
      version = '3.30',
      verbose = T,
      readAll = T
...
# clean environment
var.to.save <- c(var.to.save, 'Fore23_base_official')
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
run_SS(SS_version = '3.30.21',
      # version of SS
      base_path =Dir_23_base_official,
...
Dirplot <- file.path(Dir_23_base_official, 'run', fsep = fsep)
...
# This can be done using the 5.23_Official_Base_Outputs.R script.
