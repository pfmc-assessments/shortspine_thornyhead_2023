if(FALSE){
  ...
  starter <- SS_readstarter(file.path(dir.R0prof, 'starter.ss'))
  # change control file name in the starter file
  starter$ctlfile <- "control_modified.ss"
  # make sure the prior likelihood is calculated
  # for non-estimated quantities
  starter$prior_like <- 1
  # write modified starter file
  SS_writestarter(starter, dir=dir.R0prof, overwrite=TRUE)
  # run SS_profile command
...
#  profile <- SS_profile(dir=dir.R0prof, # directory
  ...
  prof.R0.models2 <- SSgetoutput(dirvec=dir.R0prof2, keyvec=1:Nprof.R02)
  # summarize output
  prof.R0.summary2 <- SSsummarize(prof.R0.models2)
  names(prof.R0.models2) <- paste(names(prof.R0.models2),".2",sep="")

  prof.R0.summary3 <- SSsummarize(c(prof.R0.models2[1:5],prof.R0.models[-(1:7)]))
  ...
  par(mar=c(5,4,1,1))
  SSplotProfile(prof.R0.summary3,           # summary object
                profile.string = "R0", # substring of profile parameter
                profile.label=expression(log(italic(R)[0])),ymax=5,
                pheight=4.5,print=FALSE,plotdir=dir.R0prof) # axis label
  abline(v=10.33,lty=2)
  ...
  prof.R0.summary <- prof.R0.summary3
  ...
  R0.vec3 <- as.numeric(prof.R0.summary3$pars[grep("R0",prof.R0.summary3$pars$Label),1:prof.R0.summary3$n])
  deplvec <- as.numeric(prof.R0.summary3$quants[prof.R0.summary3$quants$Label=="Bratio_2013",1:prof.R0.summary3$n])
  B0vec   <- as.numeric(prof.R0.summary3$quants[prof.R0.summary3$quants$Label=="SPB_Virgin",1:prof.R0.summary3$n]/1e3)
  B2013vec   <- as.numeric(prof.R0.summary3$quants[prof.R0.summary3$quants$Label=="SPB_2013",1:prof.R0.summary3$n])

  mods <- order(R0.vec3)
  # make plot showing B0, depletion, and Q as a function of R0
  png(file.path(dir.R0prof,"R0_profile_dependencies_SST_6.2.png"),width=7,height=9,res=300,units='in')
  par(mfrow=c(3,1),mar=c(1,4,1,1),oma=c(4,0,0,0),cex=.8)
  plot(R0.vec3[mods],B0vec[mods],type='l',lwd=3,ylim=c(0,800),las=1,yaxs='i',
       xlab="",ylab=expression(paste(italic(B)[0]," (1000 mt)")))
  points(R0.vec3[prof.R0.summary3$n],B0vec[prof.R0.summary3$n],pch=16,cex=2)
  abline(h=B0vec[prof.R0.summary3$n],lty=3)
#  axis(2,at=round(B0vec[prof.R0.summary3$n],2),las=1,padj=0)
  axis(1,at=baseval,label=baselab)
  abline(v=baseval,lty=3,col='grey')

  plot(R0.vec3[mods],deplvec[mods],type='l',lwd=3,ylim=c(0,1),las=1,yaxs='i',
       xlab=expression(log(italic(R)[0])),ylab="Spawning depletion")
  points(R0.vec3[prof.R0.summary3$n],deplvec[prof.R0.summary3$n],pch=16,cex=2)
  abline(h=deplvec[prof.R0.summary3$n],lty=3)
  axis(1,at=baseval,label=baselab)
  abline(v=baseval,lty=3,col='grey')

  plot(R0.vec3[mods],Qvec[mods],type='l',lwd=3,ylim=c(0,1.5),las=1,yaxs='i',
       xlab=expression(log(italic(R)[0])),ylab="Catchability of NWFSC Combo Survey")
  points(R0.vec3[prof.R0.summary3$n],Qvec[prof.R0.summary3$n],pch=16,cex=2)
  axis(1,at=baseval,label=baselab)
#  axis(2,at=round(Qvec[prof.R0.summary3$n],2),las=1)
  abline(h=Qvec[prof.R0.summary3$n],lty=3,col='grey')
  abline(v=baseval,lty=3,col='grey')
  dev.off()
  ...
  #########################################################
  B2013.row <- s6.2$derived_quants[s6.2$derived_quants$LABEL=="SPB_2013",]
  ##        LABEL  Value  StdDev
  ## 533 SPB_2013 141800 67441.2
  ...
  plot(R0.vec3[mods],B2013vec[mods],type='o',lwd=3,ylim=c(0,4e5),ylab="Spawning biomass in 2013",yaxs='i',axes=FALSE)
  #axis(1,at=seq(9.6,12,.2))
  axis(1,at=c(9.8,10.33,10.7),labels=c("9.8","10.33","10.7"))
  axis(2)
  box()
  ...
  low.state$likelihoods_used$values[1] - s6.2$likelihoods_used$values[1]
  #[1] 2.2
  ...
  high.state$likelihoods_used$values[1] - s6.2$likelihoods_used$values[1]
  #[1] 2.12
  high.state2$likelihoods_used$values[1] - s6.2$likelihoods_used$values[1]
  #[1] 0.509

  states <- SSsummarize(list(low.state,s6.2,high.state))
#  SSplotComparisons(states,png=TRUE,

  SSplotComparisons(states,pheight=5.5,
                    legendlabels=c("Low state","Base","High state"),
                    legendloc='bottomleft',
                    densitynames=c("SPB_2013","SPB_Virgin"),
                    png=TRUE,plotdir='../STAR/states_of_nature_July25_lunchtime')
