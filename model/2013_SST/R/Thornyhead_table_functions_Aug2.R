...
if(FALSE){ # stuff inside these section intended for pasting rather than sourcing
  ...
  dir       <- file.path(dir_folder, "SST_base_FINAL")
  dir.table <- file.path(dir_folder, "../tables/SST_base_FINAL_tables")
  dir.create(dir.table)

  dir       <- file.path(dir_folder, "LST_Base_Lmax_Est")
  dir.table <- file.path(dir_folder, "LST_Base_Lmax_Est_tables")
  dir.create(dir.table)
...
format.fn <- function(x,digits=1,scale=1,percent=FALSE){
  if(percent){
    paste(format(round(100*x/scale,digits), digits=digits, nsmall=digits,
                 scientific=FALSE),"%",sep="")
  }else{
    format(round(x/scale,digits), digits=digits, nsmall=digits, scientific=FALSE)
  }
}
...
Table2 <- function(First_year=2001, Last_year=2013, scale=1000, digits=1, write.csv=TRUE)
    {
      if(scale!=1) cat('Table2 is dividing spawning biomass by',scale,
                       'so check your units\n')
      # Question: why use ceiling and trunc here instead of round?
      ...
      Table2a$SPB <- format.fn(stdtable$Value[grep("SPB_", stdtable$LABEL)],scale=scale)
      #Table2a$SPB_95 <- ceiling(stdtable$CI9[grep("SPB_", stdtable$LABEL)])
      SPB_lo <- format.fn(stdtable$lower[grep("SPB_", stdtable$LABEL)],scale=scale)
      SPB_hi <- format.fn(stdtable$upper[grep("SPB_", stdtable$LABEL)],scale=scale)
      Table2a$CI_SPB <- paste(SPB_lo, " -" , SPB_hi, sep="")
      ...
      Table2b$Depletion    <-       format.fn(stdtable$Value[grep("Bratio_", stdtable$LABEL)],percent=TRUE)
      Table2b$CI_Depletion <- paste(format.fn(stdtable$lower[grep("Bratio_", stdtable$LABEL)],percent=TRUE), " -" ,
                                    format.fn(stdtable$upper[grep("Bratio_", stdtable$LABEL)],percent=TRUE), sep="")

      # make them a data frame
      Table2a <- as.data.frame(Table2a)
      Table2b <- as.data.frame(Table2b)
      print(range(Table2a$Year))
      print(range(Table2b$Year))
      # keep only the specified last years
      Table2 <- cbind(Table2a[which(Table2a$Year>=First_year & Table2a$Year<=Last_year),],
                      Table2b[which(Table2b$Year>=First_year & Table2b$Year<=Last_year),-1])
      # re-assign colomns names or change later from CSV file
      colnames(Table2) <- c("Fishing year", "Spawning biomass (mt)",
                            "~95% confidence interval", "Estimated depletion",
...
Table3 <- function(First_year=2001, Last_year=2013, Rec_unit="(millions)", scale=1000, digits=1, write.csv=TRUE)
    {
      Table3 <- c()
      ...
      Table3$Recruit <- format.fn(stdtable$Value,scale=scale)
      #Table3$CI95 <- ceiling(stdtable$upper - stdtable$lower)
      Table3$CI_recruit <- paste(format.fn(stdtable$lower,scale=scale), " - " ,
                                 format.fn(stdtable$upper,scale=scale), sep="")
...
# takes as input the user specified years for which to obtain this table for
Table5 <- function(First_year=2001, Last_year=2013, d=1, d.F=4, percent=TRUE, write.csv=TRUE)
    {
      sprtarg <- SSoutputs$sprtarg
      Table5 <- c()
      # extract recruitment time series
      SPRratioLabel <- SSoutputs$SPRratioLabel
      derived_quants <- SSoutputs$derived_quants
      stdtable <- derived_quants
      to_skip <- unlist(gregexpr("_", stdtable$LABEL))+1
      ...
      Table5a$SPR <- format.fn(stdtable_a$Value,          percent=percent, digits=d)
      Table5a$CI_SPR <- paste(format.fn(stdtable_a$lower, percent=percent, digits=d), " - " ,
                              format.fn(stdtable_a$upper, percent=percent, digits=d), sep="")
      Label2 <- NA
      if(substr(SPRratioLabel,1,11)=="(1-SPR)/(1-"){
        Table5a$SPR2 <-          format.fn(stdtable_a$Value*(1-sprtarg), percent=percent, digits=d)
        Table5a$CI_SPR2 <- paste(format.fn(stdtable_a$lower*(1-sprtarg), percent=percent, digits=d), " - " ,
                                 format.fn(stdtable_a$upper*(1-sprtarg), percent=percent, digits=d), sep="")
        Label2 <- "1-SPR(%)"
      }
      if(substr(SPRratioLabel,1,11)=="(1-SPR)"){
        SPRratioLabel <- "1-SPR(%)"
        Table5a$SPR2 <- NA
        Table5a$CI_SPR2 <- NA
      }
      ...
      Table5b$F <- format.fn(stdtable_b$Value,digits=d.F)
      Table5b$CI_F <- paste(format.fn(stdtable_b$lower, digits=d.F), " - " ,
                            format.fn(stdtable_b$upper, digits=d.F), sep="")
      # make them a data frame
      Table5a <- as.data.frame(Table5a)
      Table5b <- as.data.frame(Table5b)
      # keep only the specified last years
      Table5 <- cbind(Table5a[which(Table5a$Yr>=First_year & Table5a$Yr<=Last_year),],
                      Table5b[which(Table5b$Yr>=First_year & Table5b$Yr<=Last_year),-1])
      # reassign columns names
      colnames(Table5) <- c("Year", paste("Estimated",SPRratioLabel),
                            "~95% confidence interval",
                            paste("Estimated",Label2),
                            "~95% confidence interval",
                            "F",
                            "~95% confidence interval")# "Range of states of nature")
      # write the table
      if(write.csv==TRUE)
          {
...
Table6 <- function(First_year.input=2013, Last_year.input=2022, default.input=TRUE, write.csv=TRUE)
    {
      ...
      Table6$OFL <- round(Table6a[grep("OFLCatch_", Table6a$LABEL),]$Value)
      Table6$ACL <- round(Table6a[grep("ForeCatch_", Table6a$LABEL),]$Value)
      Table6$Bio3 <- round(Table6b$Bio_smry)
      Table6$SPB <- round(Table6a[grep("SPB_", Table6a$LABEL),]$Value)
      Table6$Depletion <- round(Table6a[grep("Bratio_", Table6a$LABEL),]$Value, 3)
      # make them a data frame
      Table6 <- as.data.frame(Table6)
      # reassign columns names
      colnames(Table6) <- c("Year", "OFL (mt)", "ACL (mt)", "Age 1+ biomass (mt)", "Spawning biomass (mt)", "Depletion")
      # write the table
      if(write.csv==TRUE)
          {
...
Table8 <- function(write.csv=TRUE)
    {
      # Note: the SPR and Btarget values could be made automatic based on info in the model
      SBtarg <- paste(round(100*SSoutputs$btarg),"%",sep="")
      SPRMSYproxy <- SSoutputs$sprtarg
      ...
      derived_quants <- SSoutputs$derived_quants
      Table8a <- derived_quants[c(grep("SSB_Unfished",derived_quants$LABEL, fixed=TRUE),
                                  grep("SmryBio_Unfished",derived_quants$LABEL, fixed=TRUE),
                                  grep("Recr_Unfished",derived_quants$LABEL, fixed=TRUE),
                                  grep(paste0("Bratio_", Last_year),derived_quants$LABEL, fixed=TRUE)),]
      ...
      Table8[-c(4,7,8,12,13,17,18),2:4] <- round(Table8[-c(4,7,8,12,13,17,18),2:4],0)
      Table8[c(4,7,8,12,13,17,18),2:4] <- round(Table8[c(4,7,8,12,13,17,18),2:4],6)
      # format table
...
#      tmp <- stdtable$Value[grep("Bratio_", stdtable$LABEL)]
#      tmp <- c(NA,NA,NA,tmp)
#      Table13$Depletion <- tmp
      ...
      Table13 <- cbind(Table13[which(Table13$Yr>=First_year & Table13$Yr<=Last_year),],
                       Table13a[which(Table13a$Yr>=First_year & Table13a$Yr<=Last_year),-1],
                       Table13b[which(Table13b$Yr>=First_year & Table13b$Yr<=Last_year),-1])
      # rename columns
...
makeTables <- function(write.csv=TRUE){
  ...
  Tables2 = Table2(First_year=2001, Last_year=2013, write.csv=write.csv)

  ### Recent recruitment table
  # input the first and last year for which you want this info
  # and the unit of the recruitment data
  Table3(First_year=2001, Last_year=2012, Rec_unit="(1000s)", write.csv=write.csv)

  ### Recent catch and landings
  # input the first and last year for which you want this info
  Table4(First_year=2001, Last_year=2012, write.csv=write.csv)
  ...
  ### Recent trend in SPR and F
  # input the first and last year for which you want this info
  Table5(First_year=2001, Last_year=2012, write.csv=write.csv)
  ...
  # second attempt with revised P*=0.40 and sigma=0.72
  Table6(First_year.input=2013, Last_year.input=2024, default.input=FALSE, write.csv=FALSE)

  # creating projection table for DeVore's December 2013 request
  tmp <- pstar$timeseries
  tmp <- tmp[,names(tmp)%in%c("Yr","Bio_smry","SpawnBio")]
