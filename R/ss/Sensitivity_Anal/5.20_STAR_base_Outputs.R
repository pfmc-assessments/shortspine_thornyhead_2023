...
# Path to the base model (23.model.rec_long_main_period) repertory
Dir_23_model_rec_long_main_period <- file.path(dirname(dir_SensAnal), 'STAR_Panel', '5.19_Recruitment_Requests_2/1_23.model.rec_long_main_period', 'run', fsep = fsep)

# Root directory for this sensitivity analysis
SA_path <- file.path('model','Sensitivity_Anal','STAR_Panel','5.20_STAR_base',fsep = fsep)

# Path to the 23.STAR.base repertory
Dir_23_STAR_base <- file.path(dir_SensAnal, '5.20_STAR_base','1_23.STAR.base' , 'run',fsep = fsep)

# Extract the outputs for all models
SensiMod <- SSgetoutput(dirvec = c(
	Dir_23_model_rec_long_main_period,
	Dir_23_STAR_base))
