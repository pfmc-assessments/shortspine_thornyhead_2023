...
table_caption2 <- glue::glue("Original {assess_year} projection and alternative catch-only projection in which the 2026 assumed catches are set to 67% of the ACL from a projection only through 2025 (2026 ACL in a catch-only projection through 2025 = 834 mt) rather than fixed at the values calculated by the GMT based on the current harvest specifications. See caption of table above for additional details.")
```
...
An alternative projection is provided in the second table where the assumed catch in 2026 is based on ACL from a new catch-only projection through 2025 rather than the current harvest specifications. An assumption of 67% attainment of the ACL estimated by the GMT was used in all cases.
