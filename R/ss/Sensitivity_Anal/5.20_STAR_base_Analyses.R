...
# sensitivity analysis: Item 5.20 
# ============================================================ #
# 
...
# 	- STAR Approved Base Model 
# Author:  Team Thornyhead 
# Date: 2023-06-08 13:16:07 
...
#	 /Users/jzahner/Desktop/Projects/shortspine_thornyhead_2023/model/Sensitivity_Anal/STAR_Panel/5.20_STAR_base 
# 
# Features: 
#  The official base model approved by STAR Panel on June 8, 2023. 
# ============================================================ #
...
# /Users/jzahner/Desktop/Projects/shortspine_thornyhead_2023/R/ss/Sensitivity_Anal/5.20_STAR_base_Outputs.R 
...
#  3. Developing model 23.STAR.base  ----
# ----------------------------------------------------------- #

# Path to the 23.STAR.base repertory
Dir_23_STAR_base <- file.path(dir_SensAnal, '5.20_STAR_base','1_23.STAR.base' ,fsep = fsep)

# Add the model directory to the saved variables
var.to.save <- c(var.to.save, 'Dir_23_STAR_base') 


# For each SS input file, the following variable names will be used:
# Starter file :			 Start23_STAR_base 
# Data file :			 Dat23_STAR_base 
# Control file :			 Ctl23_STAR_base 
# Forecast file :			 Fore23_STAR_base 
...
Restart_SA_modeldvpt(
      base.model = '23.model.rec_long_main_period',
      curr.model = '23.STAR.base',
      files = 'all')
...
# ======================= #
# Read in the file
StarterFile <- file.path(Dir_23_STAR_base, 'starter.ss', fsep = fsep)
Start23_STAR_base <- SS_readstarter(
      file = StarterFile,
      verbose = TRUE
      )
...
SS_writestarter(
      mylist =  Start23_STAR_base ,
      dir =  Dir_23_STAR_base, 
      overwrite = TRUE,
      verbose = TRUE
      )

# Check file structure
# StarterFile <- file.path(Dir_23_STAR_base, 'starter.ss')
#  Start23_STAR_base <- SS_readstarter(
      # file = StarterFile,
      # verbose = TRUE
      # )
...
# clean environment
var.to.save <- c(var.to.save, 'Start23_STAR_base')
...
# ======================= #
# Read in the file
DatFile <-file.path(Dir_23_STAR_base,Start23_STAR_base$datfile, fsep = fsep)
Dat23_STAR_base <- SS_readdat_3.30(
      file = DatFile,
      verbose = TRUE,
      section = TRUE
      )
...
SS_writedat(
      datlist =  Dat23_STAR_base ,
      outfile = file.path(Dir_23_STAR_base, 'SST_data.ss', fsep = fsep),
      version = '3.30',
      overwrite = TRUE
      )

# Check file structure
# DatFile <- file.path(Dir_23_STAR_base, 'SST_data.ss', fsep = fsep)
#  Dat23_STAR_base <-
      # SS_readdat_3.30(
      # file = DatFile,
      # verbose = TRUE,
...
# clean environment
var.to.save <- c(var.to.save, 'Dat23_STAR_base')
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
# Read in the file
Ctlfile <-file.path(Dir_23_STAR_base,Start23_STAR_base$ctlfile, fsep = fsep)
Ctl23_STAR_base <- SS_readctl_3.30(
      file = Ctlfile,
      use_datlist = TRUE,
      datlist = file.path(Dir_23_STAR_base, 'run','data_echo.ss_new', fsep = fsep),
      verbose = TRUE
      )
...
SS_writectl(
      ctllist =  Ctl23_STAR_base ,
      outfile = file.path(Dir_23_STAR_base, 'SST_control.ss', fsep = fsep),
      version = '3.30',
      overwrite = TRUE
      )
# Check file structure
...
# clean environment
var.to.save <- c(var.to.save, 'Ctl23_STAR_base')
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
Fore23_STAR_base <-SS_readforecast(
      file = ForeFile,
      version = '3.30',
      verbose = T,
      readAll = T
...
# clean environment
var.to.save <- c(var.to.save, 'Fore23_STAR_base')
rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
Dirplot <- file.path(Dir_23_STAR_base, 'run', fsep = fsep)
...
# This can be done using the 5.20_STAR_base_Outputs.R script.
